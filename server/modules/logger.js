...
const consoleFormat = winston.format.combine(winston.format.colorize(), winston.format.timestamp(), winston.format.align(), winston.format.printf(printf));

const fileFormat = winston.format.combine(winston.format.timestamp(), winston.format.printf(printf));
...
module.exports = (loglevel = 'info', logpath = null) => {
  ...
  const selfCheck = (message, data, rest) => {
    ...
    if (typeof data !== 'object' && typeof data !== 'undefined') {
      logger.error('LOGGER: wrong type for data', {
        type: typeof data,
        data: data
      });
    }
